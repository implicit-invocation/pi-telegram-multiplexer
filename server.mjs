...
function spawnPi(workspacePath) {
	...
	const child = spawn(command, args, { cwd: key, detached: true, stdio: "ignore", env: { ...process.env, TELEMULTI_CHILD: "1" } });
	child.unref();
	spawned.set(key, { pid: child.pid, args, startedAt: Date.now() });
	child.on("exit", () => {
		spawned.delete(key);
	});
